	...
	<div v-if="isOpen" class="fixed inset-0 flex items-center justify-center bg-gray-900 dark:bg-gray-800 bg-opacity-90 dark:bg-opacity-90">
		<UCard class="w-11/12 md:w-96 rounded shadow-lg">
			<div class="flex items-start justify-between mb-4">
				...
				<!-- 	<div class="grid">
					<UIcon name="material-symbols:list-alt-add-outline-rounded" class="text-green-600 dark:text-green-600 self-center w-6 h-6" />
					<span class="text-xs font-light pl-1">Ürün Ekleyin</span>
				</div> -->
				<UIcon name="solar:close-circle-outline" class="bg-red-500 hover:bg-black dark:bg-red-300 dark:hover:bg-white text-2xl" @click="closeModal" />
			</div>
